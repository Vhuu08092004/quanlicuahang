    ...
    public class PaymentQRController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateQRPayment([FromBody] PaymentQRCreateDto dto)
        {
            ...
            try
            {
                ...
                var payment = await _paymentService.CreatePaymentForOrderAsync(
                    ...
                    note: dto.Note ?? $"Thanh toán QR {EnumHelper.GetPaymentMethodName(paymentMethod)}",
                ...
                var response = new PaymentQRResponseDto
                {
                    ...
                    PaymentMethodName = EnumHelper.GetPaymentMethodName(paymentMethod),
                    Status = PaymentStatus.Pending.ToString(),
                    StatusName = EnumHelper.GetPaymentStatusName(PaymentStatus.Pending),
        ...
        public async Task<IActionResult> CheckPaymentStatus([FromRoute] string paymentId)
        {
            try
            {
                ...
                var statusCode = payment.PaymentStatus;
                var statusName = EnumHelper.TryParsePaymentStatus(payment.PaymentStatus, out var ps)
                    ? EnumHelper.GetPaymentStatusName(ps)
                    : payment.PaymentStatus;

                var methodCode = payment.PaymentMethod;
                var methodName = EnumHelper.TryParsePaymentMethod(payment.PaymentMethod, out var pm)
                    ? EnumHelper.GetPaymentMethodName(pm)
